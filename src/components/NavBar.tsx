...
const NavBar: React.FC<NavBarProps> = ({ routes }) => {
  return (
    <Flex align="center" justify="center" position="fixed" bottom="0" height="10vh" w="100%">
      <Box
        p="2"
        ...
        {routes.map(route => (
          <Tooltip key={route.path} label={route.name} placement="top">
            <IconButton colorScheme='black' variant='outline' as={Link} to={route.path} icon={React.createElement(Icons[route.iconName])} aria-label={route.name} mx={1} size="md" />
          </Tooltip>
        ))}
      </Box>
    </Flex>
